...
const save = async () => {
  try {
    ...
    if (form.type === 2 && !isExternalUrl.value && component) {
      component = `${component}`
    }
    ...
    const saveData = {
      ...form,
      component,
      parentId: form.parentId || 0  // 如果没有父级菜单，设为0
    }
    
    let res
    if (isEdit.value) {
      // 编辑模式，调用更新接口
      res = await baseAPI.update(saveData)
    } else {
      // 新增模式，调用新增接口
      res = await baseAPI.add(saveData)
    }
    
    if (res) {
      Message.success(isEdit.value ? '菜单更新成功' : '菜单新增成功')
      emit('save-success')
      return true
    } else {
      return false
